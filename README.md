...
## Идея для следующего шага

Если захотите, можно легко расширить этот скрипт до мини-бота с командами:

- `/price` - показать текущую цену
- `/check` - форсировать проверку
- `/status` - показать последнее сохранённое состояние

Сейчас реализован самый надёжный и простой вариант для VDS: парсер + Telegram-уведомления от бота.
