...
Если используете `STEAM_CC=kz`, скрипт автоматически попробует получить курс тенге к рублю с API Банка России и покажет примерный эквивалент в рублях в уведомлении.

Если API временно недоступен, можно добавить резервное значение:

```env
KZT_TO_RUB_RATE=0.17
```
...
- `KZT_TO_RUB_RATE` - резервный курс тенге к рублю, используется только если API Банка России недоступен
